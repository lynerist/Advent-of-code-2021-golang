...
func main(){
	...
	fmt.Println(explore("start", connections, map[string]bool{}))
}

func explore(start string, connections map[string][]string, alreadyVisited map[string]bool)int{
	if start == "end"{
		return 1
	}
	...
	for _, node := range connections[start]{
		if ! visited[node]{
			paths += explore(node, connections, visited)
		}
	} 
	return paths
}